...
function Header(props) {
  ...
  function handleSubmit(e) {
    e.preventDefault();
    navigate("/search/" + value.searchvl)
    ...
    <>
      <div className='menu'>
        ...
        <div className='menu-middle'>
          <button className='menu-DanhMuc' onClick={dropdown}>
          ...
          <form onSubmit={handleSubmit} className='menu-form-search'>
            <input type='text' placeholder='Tìm Kiếm' className='menu-search' name='searchvl' onChange={handleInput} />
          </form>
        </div>
        <div className='menu-bottom'>
          <button className='menu-Cart' onClick={handleClick}>
            ...
            {items.length > 0 ? <p className="menu-cart-count">{items.length}</p> : null }
          </button>
      ...
      {showdropdown &&
        <div className="Dropdown">
          {danhmucdata.map((item, index) => {
            return (
              <Link to={`/${item.maloaisp}`} style={{ textDecoration: "none", color: "black" }}>
